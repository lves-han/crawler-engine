...
use processor_chain::processors::processor_chain::{ErrorStrategy, TypedChain};
use std::sync::Arc;
...
impl<P> EventAwareProcessor<P> {
    ...
    async fn publish_event(&self, event: SystemEvent) {
        ...
        // if let Some((i, _)) = s.char_indices().nth(200) {
        //     s.truncate(i);
        //     s = s + "......";
        // }
        info!("Publishing event: {}", s);
    ...
    Output: Send + Sync + 'static,
{
    ...
    async fn process(&self, input: Input, context: ProcessorContext) -> ProcessorResult<Output> {
        ...
        self.inner_processor
            .process(input.clone(), context.clone())
            .await
    }
...
impl<In, ElemIn> EventAwareTypedChain<In, futures::stream::BoxStream<'static, ElemIn>>
where
    In: Send + Clone + 'static,
    ElemIn: Send + Sync + Clone + 'static,
{
    pub fn then_map_stream_in_with<ElemOut, P>(
        ...
        processor: P,
        concurrency: usize,
    ) -> EventAwareTypedChain<In, futures::stream::BoxStream<'static, ElemOut>>
    where
        ElemOut: Send + Sync + 'static,
        P: EventProcessorTrait<ElemIn, ElemOut> + Send + Sync + 'static,
    {
    ...
    pub fn then_map_stream_in_result_with<ElemOut, P>(
        ...
        processor: P,
        concurrency: usize,
    ) -> EventAwareTypedChain<In, futures::stream::BoxStream<'static, Result<ElemOut>>>
    where
        ElemOut: Send + Sync + 'static,
        P: EventProcessorTrait<ElemIn, ElemOut> + Send + Sync + 'static,
    {
...
pub trait EventProcessorTrait<Input, Output>: ProcessorTrait<Input, Output> {
    fn pre_status(&self, input: &Input) -> SystemEvent;
    fn finish_status(&self, input: &Input, output: &Output) -> SystemEvent;
    fn working_status(&self, input: &Input) -> SystemEvent;
    fn error_status(&self, input: &Input, err: &Error) -> SystemEvent;
    fn retry_status(&self, input: &Input, retry_policy: &RetryPolicy) -> SystemEvent;
}
    ...
    Output: Send + Sync + 'static,
{
    ...
    fn finish_status(&self, input: &Input, out: &Output) -> SystemEvent {
