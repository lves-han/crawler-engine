    ...
    for DownloadProcessor
{
    ...
    async fn process(
        ...
        if !is_retry {
            ...
            // 1. 检查 Task 级别错误
            match self.state.error_tracker.should_task_continue(&input.0.task_id()).await {
            ...
            // 2. 检查 Module 级别错误
            match self.state.error_tracker.should_module_continue(&input.0.module_id()).await {
                Ok(kernel::ErrorDecision::Continue) => {
        ...
        let request_id = input.0.id.clone();
...
impl EventProcessorTrait<(Request, Option<ModuleConfig>), (Option<Response>, Option<ModuleConfig>)>
    for DownloadProcessor
{
    fn pre_status(&self, input: &(Request, Option<ModuleConfig>)) -> SystemEvent {
        let ev: DownloadEvent = (&input.0).into();
        SystemEvent::Download(DownloaderCreate(ev))
    }
    ...
    fn finish_status(
        ...
        SystemEvent::Download(DownloadCompleted(ev))
    }

    fn working_status(&self, input: &(Request, Option<ModuleConfig>)) -> SystemEvent {
        let ev: DownloadEvent = (&input.0).into();
        SystemEvent::Download(DownloadStarted(ev))
    }

    fn error_status(&self, input: &(Request, Option<ModuleConfig>), err: &Error) -> SystemEvent {
        let ev: DownloadEvent = (&input.0).into();
        let failure = DynFailureEvent {
            data: ev,
            error: err.to_string(),
        };
        SystemEvent::Download(DownloadFailed(failure))
    }
    ...
    fn retry_status(
        ...
        SystemEvent::Download(DownloadRetry(retry))
    }
}
...
impl ProcessorTrait<Option<Response>, ()> for ResponsePublishProcessor {
    ...
    async fn pre_process(
        ...
        if let Some(resp) = _input {
            debug!(
                "[ResponsePublish] lock module before publish: module_id={}",
                resp.module_id()
            );
...
impl EventProcessorTrait<Option<Response>, ()> for ResponsePublishProcessor {
    fn pre_status(&self, input: &Option<Response>) -> SystemEvent {
        match input {
            Some(resp) => SystemEvent::ResponsePublish(ResponsePublishPrepared(resp.into())),
            None => SystemEvent::System(EventSystem::ErrorHandled("no_response_to_publish".into())),
        }
    }

    fn finish_status(&self, input: &Option<Response>, _out: &()) -> SystemEvent {
        match input {
            Some(resp) => SystemEvent::ResponsePublish(ResponsePublishCompleted(resp.into())),
            None => SystemEvent::System(EventSystem::ErrorHandled("no_response_to_publish".into())),
        }
    }

    fn working_status(&self, input: &Option<Response>) -> SystemEvent {
        match input {
            Some(resp) => SystemEvent::ResponsePublish(ResponsePublishSend(resp.into())),
            None => SystemEvent::System(EventSystem::ErrorHandled("no_response_to_publish".into())),
        }
    }

    fn error_status(&self, input: &Option<Response>, err: &Error) -> SystemEvent {
        match input {
            Some(resp) => {
                ...
                SystemEvent::ResponsePublish(ResponsePublishFailed(failure))
            }
            None => SystemEvent::System(EventSystem::ErrorOccurred(format!(
                "response_publish_error_without_response: {err}"
            ))),
        }
    }

    fn retry_status(&self, input: &Option<Response>, retry_policy: &RetryPolicy) -> SystemEvent {
        match input {
            Some(resp) => {
                ...
                SystemEvent::ResponsePublish(ResponsePublishRetry(retry))
            }
            None => SystemEvent::System(EventSystem::ErrorHandled(
...
{
    fn pre_status(&self, input: &(Request, Option<ModuleConfig>)) -> SystemEvent {
        SystemEvent::RequestMiddleware(RequestMiddlewarePrepared((&input.0).into()))
    }
    ...
    fn finish_status(
        ...
        SystemEvent::RequestMiddleware(RequestMiddlewareCompleted((&out.0).into()))
    }

    fn working_status(&self, input: &(Request, Option<ModuleConfig>)) -> SystemEvent {
        SystemEvent::RequestMiddleware(RequestMiddlewareStarted((&input.0).into()))
    }
    ...
    fn error_status(&self, input: &(Request, Option<ModuleConfig>), err: &Error) -> SystemEvent {
        ...
        SystemEvent::RequestMiddleware(RequestMiddlewareFailed(failure))
    }
    ...
    fn retry_status(
        ...
        SystemEvent::RequestMiddleware(RequestMiddlewareRetry(retry))
    }
}
...
impl EventProcessorTrait<(Option<Response>, Option<ModuleConfig>), Option<Response>>
    for ResponseMiddlewareProcessor
{
    fn pre_status(&self, input: &(Option<Response>, Option<ModuleConfig>)) -> SystemEvent {
        match &input.0 {
            Some(resp) => SystemEvent::ResponseMiddleware(ResponseMiddlewarePrepared(resp.into())),
            None => SystemEvent::System(EventSystem::ErrorHandled("no_response_to_process".into())),
        }
    }
    ...
    fn finish_status(
        ...
        match out {
            Some(resp) => SystemEvent::ResponseMiddleware(ResponseMiddlewareCompleted(resp.into())),
            None => SystemEvent::System(EventSystem::ErrorHandled("no_response_to_process".into())),
        }
    }

    fn working_status(&self, input: &(Option<Response>, Option<ModuleConfig>)) -> SystemEvent {
        match &input.0 {
            Some(resp) => SystemEvent::ResponseMiddleware(ResponseMiddlewareStarted(resp.into())),
            None => SystemEvent::System(EventSystem::ErrorHandled("no_response_to_process".into())),
        }
    }
    ...
    fn error_status(
        ...
        match &input.0 {
            Some(resp) => {
                ...
                SystemEvent::ResponseMiddleware(ResponseMiddlewareFailed(failure))
            }
            None => SystemEvent::System(EventSystem::ErrorOccurred(format!(
    ...
    fn retry_status(
        ...
        match &input.0 {
            Some(resp) => {
                ...
                SystemEvent::ResponseMiddleware(ResponseMiddlewareRetry(retry))
            }
            None => SystemEvent::System(EventSystem::ErrorHandled(
...
{
    fn pre_status(&self, input: &(Request, Option<ModuleConfig>)) -> SystemEvent {
        SystemEvent::Proxy(ProxyPrepared((&input.0).into()))
    }
    ...
    fn finish_status(
        ...
        SystemEvent::Proxy(ProxyCompleted((&out.0).into()))
    }

    fn working_status(&self, input: &(Request, Option<ModuleConfig>)) -> SystemEvent {
        SystemEvent::Proxy(ProxyStarted((&input.0).into()))
    }
    ...
    fn error_status(&self, input: &(Request, Option<ModuleConfig>), err: &Error) -> SystemEvent {
        ...
        SystemEvent::Proxy(ProxyFailed(failure))
    }
    ...
    fn retry_status(
        ...
        SystemEvent::Proxy(ProxyRetry(retry))
    }
}
