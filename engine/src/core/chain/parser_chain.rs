...
impl EventProcessorTrait<Response, (Response, Arc<Module>)> for ResponseModuleProcessor {
    fn pre_status(&self, input: &Response) -> SystemEvent {
        SystemEvent::ResponseModuleLoad(EventResponseModuleLoad::ModuleGenerate(input.into()))
    }

    fn finish_status(&self, input: &Response, _output: &(Response, Arc<Module>)) -> SystemEvent {
        SystemEvent::ResponseModuleLoad(EventResponseModuleLoad::ModuleGenerateCompleted(
            input.into(),
        ))
    }

    fn working_status(&self, input: &Response) -> SystemEvent {
        SystemEvent::ResponseModuleLoad(EventResponseModuleLoad::ModuleGenerateStarted(
            input.into(),
        ))
    }

    fn error_status(&self, input: &Response, err: &Error) -> SystemEvent {
        let event: ResponseModuleLoad = input.into();
        let failure = DynFailureEvent {
            data: event,
            error: err.to_string(),
        };
        SystemEvent::ResponseModuleLoad(ModuleGenerateFailed(failure))
    }

    fn retry_status(&self, input: &Response, retry_policy: &RetryPolicy) -> SystemEvent {
        let event: ResponseModuleLoad = input.into();
        ...
        SystemEvent::ResponseModuleLoad(ModuleGenerateRetry(retry))
    }
}
...
impl ProcessorTrait<(Response, Arc<Module>), Vec<Data>> for ResponseParserProcessor {
    ...
    async fn process(
        ...
        let mut data = match data {
            Ok(d) => {
                debug!(
                    "[ResponseParserProcessor] parser returned: data_len={}",
                    d.data.len()
                );
...
impl EventProcessorTrait<(Response, Arc<Module>), Vec<Data>> for ResponseParserProcessor {
    fn pre_status(&self, input: &(Response, Arc<Module>)) -> SystemEvent {
        SystemEvent::Parser(EventParser::ParserReceived((&input.0).into()))
    }

    fn finish_status(&self, input: &(Response, Arc<Module>), _output: &Vec<Data>) -> SystemEvent {
        SystemEvent::Parser(EventParser::ParserCompleted((&input.0).into()))
    }

    fn working_status(&self, input: &(Response, Arc<Module>)) -> SystemEvent {
        SystemEvent::Parser(EventParser::ParserStarted((&input.0).into()))
    }

    fn error_status(&self, input: &(Response, Arc<Module>), err: &Error) -> SystemEvent {
        let event: ParserEvent = (&input.0).into();
        let failure = DynFailureEvent {
            data: event,
            error: err.to_string(),
        };
        SystemEvent::Parser(ParserFailed(failure))
    }
    ...
    fn retry_status(
        ...
        SystemEvent::Parser(EventParser::ParserRetry(retry))
    }
}
...
impl EventProcessorTrait<Data, Data> for DataMiddlewareProcessor {
    fn pre_status(&self, input: &Data) -> SystemEvent {
        SystemEvent::DataMiddleware(EventDataMiddleware::MiddlewareBefore(input.into()))
    }

    fn finish_status(&self, input: &Data, output: &Data) -> SystemEvent {
        let mut event: DataMiddlewareEvent = input.into();
        event.after_size = output.size().into();
        SystemEvent::DataMiddleware(EventDataMiddleware::MiddlewareCompleted(event))
    }

    fn working_status(&self, input: &Data) -> SystemEvent {
        SystemEvent::DataMiddleware(EventDataMiddleware::MiddlewareStarted(input.into()))
    }

    fn error_status(&self, input: &Data, err: &Error) -> SystemEvent {
        let event: DataMiddlewareEvent = input.into();
        let failure = DynFailureEvent {
            data: event,
            error: err.to_string(),
        };
        SystemEvent::DataMiddleware(EventDataMiddleware::MiddlewareFailed(failure))
    }

    fn retry_status(&self, input: &Data, retry_policy: &RetryPolicy) -> SystemEvent {
        let event: DataMiddlewareEvent = input.into();
        let retry = DynRetryEvent {
            data: event,
            retry_count: retry_policy.current_retry,
        ...
        SystemEvent::DataMiddleware(EventDataMiddleware::MiddlewareRetry(retry))
    }
}
...
impl ProcessorTrait<Data, ()> for DataStoreProcessor {
    ...
    async fn process(&self, input: Data, context: ProcessorContext) -> ProcessorResult<()> {
        debug!(
            "[DataStoreProcessor] start store: account={} platform={} module={} size={}",
            input.account,
            input.platform,
            input.module,
            input.size()
...
impl EventProcessorTrait<Data, ()> for DataStoreProcessor {
    fn pre_status(&self, input: &Data) -> SystemEvent {
        SystemEvent::DataStore(EventDataStore::StoreBefore(input.into()))
    }

    fn finish_status(&self, input: &Data, _output: &()) -> SystemEvent {
        SystemEvent::DataStore(EventDataStore::StoreCompleted(input.into()))
    }

    fn working_status(&self, input: &Data) -> SystemEvent {
        SystemEvent::DataStore(EventDataStore::StoreStarted(input.into()))
    }
    ...
    fn error_status(&self, input: &Data, err: &Error) -> SystemEvent {
        ...
        SystemEvent::DataStore(EventDataStore::StoreFailed(failure))
    }
    ...
    fn retry_status(&self, input: &Data, retry_policy: &RetryPolicy) -> SystemEvent {
        ...
        SystemEvent::DataStore(EventDataStore::StoreRetry(retry))
    }
}
