...
use crate::core::register_module::ModuleLoader;
...
///
/// 当前的设计里，downloader默认情况下在同一个task_id里是串行下载
...
impl Engine {
    pub async fn new(state: Arc<State>) -> Self {
        ...
        let redis_queue = RedisQueue::new(&state.config.read().await.channel_redis);
        let queue_manager = QueueManager::new(Some(Arc::new(redis_queue)));
        ...
        // middleware_manager
        //     .register_download_middleware_from_vec(middleware::register_download_middlewares())
        //     .await;
        // middleware_manager
        //     .register_data_middleware_from_vec(middleware::register_data_middlewares())
        //     .await;
        // middleware_manager
        //     .register_store_middleware_from_vec(middleware::register_data_store_middlewares())
        //     .await;
        // let module_loader = Arc::new(ModuleLoader::default());
        // let module_path = state.config.read().await.crawler.path.clone();
        // module_loader.load(Path::new(&module_path), task_manager.clone()).await;
        Self {
    ...
    async fn setup_event_handlers(&self) {
        ...
        if let Some(redis_config) = &self.state.config.read().await.event_redis
            && let Some(pool) = create_redis_pool(redis_config)
        {
            let redis_handler = RedisEventHandler::new(
                Arc::new(pool),
                "crawler_engine_event".to_string(), // 可以从配置中读取
                    ...
                    match account_task {
                        Some(account_task) => {
                            ...
                            tokio::spawn(async move {
                                chain.execute(account_task, ProcessorContext::default()).await;
                            });
                        }
                        None => {
    ...
    async fn start_download_processor(&self) {
        ...
        let wss_download_chain = Arc::new(create_wss_download_chain(
            self.downloader_manager.clone(),
            self.queue_manager.clone(),
            self.middleware_manager.clone(),
            self.state.sync_service.clone(),
                    ...
                    match request {
                        Some(request) => {
                            if request.downloader.eq_ignore_ascii_case("wss_downloader") {
                                ...
                                tokio::spawn(async move {
                                    chain.execute(request, ProcessorContext::default()).await;
                                });
                            } else {
                ...
                task = rx.recv() => {
                    match task {
                        Some(task) => {
                            ...
                            tokio::spawn(async move {
                                chain.execute(task, ProcessorContext::default()).await;
                            });
                        }
                        None => {
                ...
                response = rx.recv() => {
                    match response {
                        Some(response) => {
                            ...
                            tokio::spawn(async move {
                                chain.execute(response, ProcessorContext::default()).await;
                            });
                        }
                        None => {
