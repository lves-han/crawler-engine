...
use error::{DownloadError, Error, RequestError, Result};
use kernel::model::cookies::CookieItem;
use kernel::model::download_config::DownloadConfig;
use kernel::model::headers::HeaderItem;
use kernel::model::{Cookies, Headers, Request, Response};
...
impl RequestDownloader {
    ...
    async fn do_download(&self, request: Request) -> Result<Response> {
        ...
        loop {
            let rate_limit_enabled = *self.enable_rate_limit.read().await;
...
impl Downloader for RequestDownloader {
    ...
    async fn download(&self, request: Request) -> Result<Response> {
        ...
        if locker_enabled {
            ...
            match self
                .locker
                .acquire_lock(&key, 30, Duration::from_secs(5)) // 减少TTL和超时时间
                .await
            {
                Ok(_) => {
