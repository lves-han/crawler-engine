...
// - 生成失败（execute_request）时：若存在前置请求可回退，则最多回退一次（Redis SETNX+TTL 门闸），否则直接报错。
...
// - 若解析成功但未返回 ParserTaskModel 且存在下一步，使用一次性门闸合成“占位任务”推进下一步，避免多实例重复推进。

use crate::model::ExecutionMark;
use crate::module::ModuleNodeTrait;
use crate::sync::sync_service::SyncService;
use crate::{LoginInfo, ModuleConfig, ParserData, Request, Response};
...
impl ModuleProcessorWithChain {
    ...
    fn request_field() -> &'static str {
        "request"
    }
    fn requeue_field() -> &'static str {
        "requeue"
    }
    fn advance_field(step_idx: usize) -> String {
        format!("advance_{step_idx}")
    }
    fn fallback_field(step_idx: usize, prefix: &Uuid) -> String {
        format!("fallback_{step_idx}_{prefix}")
    }
    fn advance_key(&self) -> String {
        format!("run:{}:module:{}", self.run_id, self.module_id)
    }
    fn stop_field() -> &'static str {
        "stop"
    }
    ...
    async fn save_request(&self, req: &Request) -> Result<()> {
        ...
        let val = serde_json::to_value(req)
            .map_err(|e| {
                warn!(
                    "[chain] module={} run={} save_request: serialize failed for id={}, err={}",
                    self.module_id, self.run_id, id, e
                );
                RequestError::BuildFailed(format!("Failed to serialize request: {}", e).into())
            })?;
        
        self.state
            .get_sync()
            .send(&id, Self::request_field(), &val)
            .await
            .map_err(|e| {
                warn!(
                    "[chain] module={} run={} save_request: persist failed for id={}, err={}",
                    self.module_id, self.run_id, id, e
                );
                e
            })?;
    ...
    async fn try_mark_step_advanced_once(&self, step_idx: usize) -> Result<bool> {
        // 先尝试使用 Redis 原子 SETNX + TTL 做一次性标记（分布式安全）
        let key = self.advance_key();
        let field = Self::advance_field(step_idx);
        debug!(
            "[chain] module={} run={} try_mark_step_advanced_once: key={} field={} ttl={}s",
            self.module_id, self.run_id, key, field, self.ttl
        );
        let won = self
            .state
            .get_sync()
            .setnx(&key, &field, &serde_json::json!(true), self.ttl)
            .await?;
        debug!(
    ...
    async fn try_allow_fallback_once(&self, step_idx: usize, prefix: &Uuid) -> Result<bool> {
        let key = self.advance_key();
        let field = Self::fallback_field(step_idx, prefix);
        debug!(
            "[chain] module={} run={} try_allow_fallback_once: key={} field={} ttl={}s",
            self.module_id, self.run_id, key, field, self.ttl
        );
        let allowed = self
            .state
        ...
        debug!(
            ...
            self.module_id, self.run_id, step_idx, prefix, allowed
        );
        Ok(allowed)
    }
    ...
    async fn load_request(&self, id: &Uuid) -> Result<Option<Request>> {
        ...
        let v = self
            .state
            .get_sync()
            .sync(&id_str, Self::request_field())
            .await?;
        if let Some(v) = v {
            ...
            debug!(
                "[chain] module={} run={} load_request: id={} hit",
    ...
    pub async fn mark_requeue(&self, id: &Uuid) -> Result<Option<Request>> {
        ...
        if let Some(req) = self.load_request(id).await? {
            let val = serde_json::to_value(&req)
                .map_err(|e| {
                    warn!(
                        "[chain] module={} run={} mark_requeue: serialize failed for id={}, err={}",
                        self.module_id, self.run_id, id, e
                    );
                    RequestError::BuildFailed(format!("Failed to serialize request for requeue: {}", e).into())
                })?;
            
            self.state
                .get_sync()
                .send(&id.to_string(), Self::requeue_field(), &val)
                .await?;
    ...
    async fn query_active_gates(&self) -> Result<Vec<GateInfo>> {
        ...
        // 目前返回空列表，具体实现需要根据 SyncService 的 API
        debug!(
        ...
        // TODO: 实现实际的门闸查询逻辑
        // 例如: self.state.get_sync().scan_fields(&key).await
    ...
    async fn resolve_execution_context(
        ...
        if should_infer_from_prefix {
            ...
            if let Some(prev) = self.load_request(&prefix).await? {
    ...
    async fn execute_request_impl(
            ...
            .await
        {
            Ok(mut reqs) => {
                debug!(
                    "[chain] module={} run={} execute_request_impl: generated {} request(s) for step {}",
                    self.module_id,
                    self.run_id,
                    reqs.len(),
                    step_idx
                ...
                for req in reqs.iter_mut() {
                    ...
                    // persist by request.id
                    self.save_request(req).await?;
                    debug!(
                        "[chain] module={} run={} execute_request_impl: saved request id={} step={} prefix={}",
                        self.module_id,
                        self.run_id,
                        req.id,
                        req.context.step_idx.unwrap_or(0),
    ...
    pub async fn execute_parser(
        ...
        match step_node.parser(response.clone(), config).await {
                ...
                if let Some(ref mut task) = data.parser_task {
                    ...
                    if next_idx < total {
                        ...
                        if self.try_mark_step_advanced_once(step_idx).await? {
                            let base: crate::model::message::ParserTaskModel = (&response).into();
            ...
            Err(e) => {
                ...
                let error_task = crate::model::message::ErrorTaskModel {
                    id: response.id,
                    account_task: crate::model::message::TaskModel {
    ...
    async fn is_stopped(&self) -> Result<bool> {
        ...
        let key = self.advance_key();
        let val = self.state.get_sync().sync(&key, Self::stop_field()).await?;
    ...
    async fn set_stopped(&self) -> Result<()> {
        ...
        *self.stop.write().await = true;
        ...
        let key = self.advance_key();
        self.state.get_sync().send(&key, Self::stop_field(), &serde_json::json!(true)).await?;
        debug!(
