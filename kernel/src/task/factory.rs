...
pub struct TaskFactory {
    ...
    cookie_sync_service: Arc<SyncService>,
...
impl TaskFactory {
    pub fn new(
        ...
        Self {
            ...
            cookie_sync_service,
            module_assembler,
            cache: Arc::new(tokio::sync::RwLock::new(HashMap::new())), // In-memory cache for Tasks keyed by Task::id() => "account-platform"
            state,
        }
    }
    pub async fn login_info(&self, id: &str) -> Option<LoginInfo> {

        LoginInfo::sync(id, Arc::clone(&self.cookie_sync_service.synchronizer))
            .await
            .ok()
            .flatten()
    }
    ...
    pub async fn create_task_from_model(&self, task_model: &TaskModel) -> Result<Task> {
        ...
        if let Some(names) = &task_model.module && !names.is_empty() {
    ...
    async fn create_task_with_modules(
        ...
        for module in modules {
            // 加载模块关联
            let rel_module_platform = self
                .repository
                .load_module_platform_relation(module.id, platform.id)
                .await?;
            let rel_module_account = self
                .repository
                .load_module_account_relation(module.id, account.id)
                .await?;
            ...
            let locker = module_config
                ...
                .unwrap_or(false);
            let cache_ttl = self.state.config.read().await.defaults.config.cache_ttl;
            ...
            Ok(mut task) => {
                ...
                for module in task.modules.iter_mut() {
                    let (error_times, finished) =
                        self.sync_service.load_module_status(&module.id()).await;
                    ...
                    module
                        .config
                        .send(&module.id(), self.sync_service.get_sync())
                        .await
                        .ok();
    ...
    pub async fn load_parser_model(&self, parser_model: &ParserTaskModel) -> Result<Task> {
        ...
        task.metadata = parser_model
            ...
            .unwrap_or_default();
        for module in task.modules.iter_mut() {
            let (error_times, _) = self.sync_service.load_module_status(&module.id()).await;
            module.error_times = error_times;
            ...
            if let Ok(Some(config)) =
                ModuleConfig::sync(&module.id(), self.sync_service.get_sync()).await
            {
    ...
    async fn get_from_cache(&self, key: &str) -> Option<Task> {
        // 快速读锁检查 -> 命中则克隆后刷新登录信息
        let hit = {
        ...
        if let Some(mut task) = hit {
            // 刷新 login_info（避免持锁 await）
            task.login_info = self.login_info(&task.id()).await;
            return Some(task);
        }
        // 已过期或不存在，升级为写锁清理
    ...
    async fn put_task_aliases(&self, task: Task) {
        let entry = CacheEntry {
            task,
            expires_at: Instant::now() + CACHE_TTL,
        };
        let mut cache = self.cache.write().await;
        cache.insert(entry.task.id(), entry);
    }
}
