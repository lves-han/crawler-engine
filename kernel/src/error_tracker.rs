...
impl ErrorTracker {
    pub fn new(sync_service: Arc<SyncService>, config: ErrorTrackerConfig) -> Self {
        Self {
            sync_service,
            config,
        }
    ...
    pub async fn record_download_error(
        ...
        self.increment_error(&request_key, category, severity).await?;
        let request_errors = self.get_error_count(&request_key).await?;

        // 2. 记录 Module 级别错误
        let module_key = format!("module:{}:download", module_id);
        self.increment_error(&module_key, category, severity).await?;
        ...
        if module_errors >= self.config.module_max_errors {
            // Module 达到错误阈值，释放锁
            self.release_module_lock(module_id).await;
    ...
    pub async fn record_download_success(
        ...
        if self.config.enable_success_decay {
            self.decrement_error(&request_key, self.config.success_decay_amount).await?;
        }
    ...
    pub async fn record_parse_error(
        ...
        log::info!(
            ...
            task_id, module_id, request_id, error
        );
        ...
        log::info!(
            ...
            task_errors, self.config.task_max_errors,
            module_errors, self.config.module_max_errors,
            request_errors, self.config.parse_max_retries
        );
    ...
    pub async fn should_module_continue(&self, module_id: &str) -> Result<ErrorDecision> {
        ...
        if self.is_module_terminated(module_id).await? {
            ...
            return Ok(ErrorDecision::Terminate(
                format!("Module {} has been terminated", module_id)
            ));
        }
        ...
        log::debug!(
            ...
            module_id, total_errors, self.config.module_max_errors,
            download_errors, parse_errors
        );
    ...
    /// - Module 处理完成时
    /// - Module 被手动终止时
    pub async fn release_module_lock(&self, module_id: &str) {
        self.sync_service.release_module_locker(module_id).await;
    }
    ...
    pub async fn get_stats(&self, key: &str) -> Result<ErrorStats> {
        let stats_key = format!("{}:stats", key);
        
        let stats_json = self.sync_service.synchronizer
            .sync(&stats_key, "data")
            .await?;
    ...
    pub async fn mark_task_terminated(&self, task_id: &str) -> Result<()> {
        let key = format!("task:{}:terminated", task_id);
        self.sync_service.synchronizer
            .send_with_ttl(&key, "terminated", &serde_json::json!(true), self.config.error_ttl)
            .await?;
        Ok(())
    }
    ...
    pub async fn is_task_terminated(&self, task_id: &str) -> Result<bool> {
        let key = format!("task:{}:terminated", task_id);
        if let Ok(Some(value)) = self.sync_service.synchronizer.sync(&key, "terminated").await {
    ...
    pub async fn mark_module_terminated(&self, module_id: &str) -> Result<()> {
        let key = format!("module:{}:terminated", module_id);
        self.sync_service.synchronizer
            .send_with_ttl(&key, "terminated", &serde_json::json!(true), self.config.error_ttl)
            .await?;
        Ok(())
    ...
    async fn increment_error(
        ...
        // 1. 递增总错误计数（带 TTL），直接使用 Redis 原子操作返回的新值
        // 这样可以避免并发下的“读取-修改-写入”竞态条件
        let _new_total = self.sync_service.synchronizer
            .record_error_with_ttl(key, self.config.error_ttl)
            .await?;
        
        // 2. 更新统计信息（也带 TTL）
        // 注意：这里的统计信息更新仍然可能存在并发覆盖问题，但主要用于展示，
        // 核心的熔断逻辑依赖于上面的原子计数，所以是可以接受的。
        let mut stats = self.get_stats(key).await?;
        
        // 使用 Redis 返回的原子计数更新本地统计，确保 total_errors 相对准确
        // 但由于 get_stats 和 record_error 之间有时间差，这里只是尽力而为
        // 如果需要绝对准确的 stats，需要将整个 stats 结构体用 Lua 脚本维护，或者拆分字段存储
        // 目前方案：total_errors 以 Redis 原子计数为准（用于熔断），stats 仅作参考
        ...
        *stats.errors_by_category.entry(category).or_insert(0) += 1;
        ...
        log::debug!(
            "[ErrorTracker] increment_error: key={} category={:?} total_errors={} consecutive={}",
            key, category, stats.total_errors, stats.consecutive_errors
        );
    ...
    async fn increment_success(&self, key: &str) -> Result<()> {
        ...
        self.save_stats(key, &stats).await?;
        Ok(())
    }
    ...
    async fn get_error_count(&self, key: &str) -> Result<usize> {
        // 直接读取 Redis 中的原子计数器
        // 注意：这里读取的是 "error_times" 字段，对应 record_error 操作的 key
        // RedisSync::get_key 会拼接前缀，这里我们需要手动构造一下或者复用 sync 接口
        // 由于 RedisSync::record_error 内部使用的是 "error_times" 后缀
        // 所以这里 sync(key, "error_times") 是正确的
        if let Ok(Some(count)) = self.sync_service.synchronizer.sync(key, "error_times").await {
    ...
    async fn save_stats_with_ttl(&self, key: &str, stats: &ErrorStats) -> Result<()> {
        ...
        let stats_json = serde_json::to_value(stats)
            .map_err(|e| error::SyncError::SerdeFailed(e.to_string().into()))?;
        
        self.sync_service.synchronizer
            .send_with_ttl(&stats_key, "data", &stats_json, self.config.error_ttl)
            .await?;
