#![allow(unused)]
use error::Result;
use crate::middleware::data_middleware::DataStoreMiddleware;
use crate::middleware::{DataMiddleware, DownloadMiddleware};
...
impl MiddlewareManager {
    ...
    pub async fn handle_request(&self, request: Request, config: &Option<ModuleConfig>) -> Request {
        ...
        for (middleware, _) in middleware {
            req = middleware.handle_request(req, config).await;
        }
        req
    }
    pub async fn handle_response(
        ...
        for (middleware, _) in middleware {
            if resp.download_middleware.contains(&middleware.name()) {
                resp = middleware.handle_response(resp, config).await;
            }
        }
        resp
    }
