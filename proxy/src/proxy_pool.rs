...
impl ProxyItem {
    pub fn new_for_tunnel(tunnel: Tunnel) -> Self {
        let datetime = OffsetDateTime::parse(&tunnel.expire_time, &Rfc3339).unwrap();

        let expire_time = datetime - OffsetDateTime::from_unix_timestamp(0).unwrap(); // 默认360天
        ...
        Self {
            ...
            success_count: 0,
            last_used: None,
            expire_time: expire_time.unsigned_abs(),
            provider_name: name,
            response_time: None,
            success_rate: 1.0,
    ...
    pub fn new_for_ip_proxy(ip_proxy: IpProxy, ip_provider: &IpProvider) -> Self {
        let expire_time = Duration::from_secs(ip_provider.proxy_expire_time)
            + SystemTime::now().duration_since(UNIX_EPOCH).unwrap(); // 默认5分钟
        Self {
    ...
    pub fn record_success(&mut self, response_time: Duration) {
        ...
        self.last_used = Some(SystemTime::now().duration_since(UNIX_EPOCH).unwrap());
