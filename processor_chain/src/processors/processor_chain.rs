    ...
    P: ProcessorTrait<Input, Output> + Send + Sync,
{
    async fn execute(
        ...
        // 2) 准备输入与上下文；首轮不克隆，后续重试时再从备份克隆，避免不必要的 clone 开销
        let mut current_context = context;
        let mut first_attempt_input: Option<Input> = Some(typed_input);
        let backup_input: Option<Input> = first_attempt_input.clone();

        // 2.1 should_process 判定（修复原逻辑：true 表示应处理；false 表示跳过）
        // 跳过场景：仅当 Input 与 Output 类型相同（透传）。否则只能继续处理以避免类型断裂。
        // should_process 需要一次异步调用；避免在后续 loop 内重复调用
        let mut skip_processing = false;
        if let Some(ref original) = backup_input {
            let should_do = self.processor.should_process(original, &current_context).await;
            if !should_do && TypeId::of::<Input>() == TypeId::of::<Output>() {
        ...
        if skip_processing {
            if let Some(ref original) = backup_input {
                ...
                return ProcessorResult::Success(Box::new(cloned) as Box<dyn Any + Send>);
            }
        }

        // 2.2 pre_process 在第一次尝试之前执行
        if let Some(ref original) = first_attempt_input {
            if let Err(e) = self.processor.pre_process(original, &current_context).await {
        ...
        loop {
            // 3) 选择本次尝试的输入：首轮消耗原始值，后续从备份克隆
            let this_input: Input = if let Some(v) = first_attempt_input.take() {
                v
            } else if let Some(ref original) = backup_input {
                original.clone()
            } else {
                // 理论上不会发生
                return ProcessorResult::FatalFailure(
                    ProcessorChainError::Unexpected(
                        "internal error: missing input for retry".into(),
                    )
                    .into(),
                );
            };
            ...
            // should_process 已在进入前判断；此处不再重复。
            // 5) 执行处理器（可选超时）
            let fut = self.processor.process(this_input.clone(), current_context.clone());
            ...
            match result {
                ProcessorResult::Success(output) => {
                    ...
                    if let Err(post_err) = self.processor.post_process(&this_input, &output, &current_context).await {
                        error!("Post-processing failed for {}: {}", self.name(), post_err);
                        // 将 post_err 当作 Fatal 交给 handle_error（模拟与 process Fatal 一致的路径）
                        let handled = if let Some(ref original) = backup_input {
                            self.processor.handle_error(original, post_err, &current_context).await
                        } else {
                            ProcessorResult::FatalFailure(post_err)
                        };
                        return match handled {
                            ProcessorResult::Success(out2) => ProcessorResult::Success(Box::new(out2) as Box<dyn Any + Send>),
                    ...
                    return ProcessorResult::Success(Box::new(output) as Box<dyn Any + Send>);
                }
                ProcessorResult::RetryableFailure(mut retry_policy) => {
                    ...
                    // 检查是否应该重试（根据策略返回下一次重试延迟；None 表示放弃）
                    if let Some(delay_ms) = retry_policy.delay_next_retry().await {
                        ...
                        let err = ProcessorChainError::MaxRetriesExceeded(
                            ...
                            .into();
                        let handled = if let Some(ref original) = backup_input {
                            // Use the original input reference preserved for retries
                            self.processor
                                .handle_error(original, err, &current_context)
                                .await
                        } else {
                            // Fallback: no original input available (shouldn't happen)
                ...
                ProcessorResult::FatalFailure(err) => {
                    ...
                    let handled = if let Some(ref original) = backup_input {
                        self.processor.handle_error(original, err, &current_context).await
                    } else {
                        ProcessorResult::FatalFailure(err)
                    };
                    return match handled {
                        ProcessorResult::Success(output) => ProcessorResult::Success(Box::new(output) as Box<dyn Any + Send>),
                        ProcessorResult::RetryableFailure(policy) => ProcessorResult::RetryableFailure(policy),
                        ProcessorResult::FatalFailure(e) => ProcessorResult::FatalFailure(e),
                    };
                }
        ...
        match self.mode {
            VecMapMode::Sequential => {
                ...
                for (idx, elem) in vec_input.into_iter().enumerate() {
                    match self
                        .inner
                        .execute(Box::new(elem) as Box<dyn Any + Send>, context.clone())
                        .await
                    {
                        ProcessorResult::Success(out_any) => match out_any.downcast::<ElemOut>() {
                            Ok(v) => outputs.push(*v),
                            Err(_) => {
                                return ProcessorResult::FatalFailure(
                                    ProcessorChainError::DowncastFailure(
                                        format!("Vec map step {}: element {} output type mismatch, expected {}", self.name(), idx, std::any::type_name::<ElemOut>()).into()
                                    ).into()
                                );
                            }
                        },
            ...
            VecMapMode::Parallel(concurrency) => {
                ...
                let results: Vec<(usize, ProcessorResult<Box<dyn Any + Send>>)> =
                    stream::iter(vec_input.into_iter().enumerate())
                        .map(|(idx, elem)| {
                            ...
                            async move {
                                let res = self
                                    .inner
                                    .execute(Box::new(elem) as Box<dyn Any + Send>, ctx)
                                    .await;
                                (idx, res)
                            }
                        })
                ...
                if let Some(strategy) = self.strategy {
                    ...
                    for (idx, res) in results.into_iter() {
                        ...
                        match res {
                            ProcessorResult::Success(out_any) => {
                                match out_any.downcast::<ElemOut>() {
                                    Ok(v) => outputs.push(*v),
                                    Err(_) => {
                                        error!(
                                            "parallel vec map: [{}] output type mismatch at step {}",
                                            idx,
                                            self.name()
                                        );
                                        match strategy {
                            ...
                            ProcessorResult::Success(out_any) => {
                                match out_any.downcast::<ElemOut>() {
                                    ...
                                    Err(_) => {
                                        return ProcessorResult::FatalFailure(
                                            ProcessorChainError::DowncastFailure(
                                                format!("Parallel vec map step {}: output type mismatch, expected {}", self.name(), std::any::type_name::<ElemOut>()).into()
                                            ).into()
                                        );
                                    }
                                }
...
/// 流式 map 执行器（输入为 BoxStream<ElemIn>，输出为 BoxStream<ElemOut>），内部支持并发
///
/// 支持三种错误策略：
/// - Drop：丢弃错误项，继续流；
/// - Stop：遇到错误立即停止后续产出；
    ...
    P: ProcessorTrait<ElemIn, ElemOut> + Send + Sync + 'static,
{
    async fn execute(
        ...
        let stream_in = match input.downcast::<BoxStream<'static, ElemIn>>() {
            Ok(b) => *b,
            Err(_) => {
                return ProcessorResult::FatalFailure(
                    ProcessorChainError::DowncastFailure(
                        format!(
                            "Type mismatch for stream map step {}: expected input BoxStream<{}>",
                            self.name(),
                            std::any::type_name::<ElemIn>()
                        )
                        .into(),
        ...
        let mapped = stream_in
            .map(move |elem| {
                ...
                async move {
                    inner
                        .execute(Box::new(elem) as Box<dyn Any + Send>, ctx2)
                        .await
                }
            })
                        ...
                        match res {
                            ProcessorResult::Success(out_any) => {
                                match out_any.downcast::<ElemOut>() {
                                    Ok(v) => Some(*v),
                                    Err(_) => {
                                        error!("stream map: output type mismatch");
            ...
            .boxed();
        // mapped 现为 BoxStream<'static, ElemOut>
    ...
    P: ProcessorTrait<ElemIn, ElemOut> + Send + Sync + 'static,
{
    async fn execute(
        ...
        let stream_in = match input.downcast::<BoxStream<'static, ElemIn>>() {
            ...
            Err(_) => {
                return ProcessorResult::FatalFailure(
                    ProcessorChainError::DowncastFailure(
                        format!(
                            "Type mismatch for stream map (result) step {}: expected input BoxStream<{}>",
                            self.name(),
                            std::any::type_name::<ElemIn>()
                        ).into(),
                    ).into(),
        ...
        let mapped = stream_in
            ...
            .buffer_unordered(conc)
            .map(|res| match res {
                ProcessorResult::Success(any) => match any.downcast::<ElemOut>() {
                    Ok(v) => Ok(*v),
                    Err(_) => Err(E::from(ProcessorChainError::DowncastFailure(
                        "stream map result: output type mismatch".into(),
                    ))),
                },
...
impl ProcessorChain {
    ...
    /// 添加流式 map（输入 `BoxStream<ElemIn>`，输出 `BoxStream<ElemOut>`），自定义并发度
        ...
        P: ProcessorTrait<ElemIn, ElemOut> + Send + Sync + 'static,
    {
        ...
        self.steps.push(ProcessorStep {
            name,
            executor: Box::new(executor),
            input_type_id: TypeId::of::<BoxStream<'static, ElemIn>>(),
            output_type_id: TypeId::of::<BoxStream<'static, ElemOut>>(),
            input_type_name: std::any::type_name::<BoxStream<'static, ElemIn>>(),
            output_type_name: std::any::type_name::<BoxStream<'static, ElemOut>>(),
        });
        self
    }
        ...
        P: ProcessorTrait<ElemIn, ElemOut> + Send + Sync + 'static,
    {
        ...
        self.steps.push(ProcessorStep {
            ...
            executor: Box::new(executor),
            input_type_id: TypeId::of::<BoxStream<'static, ElemIn>>(),
            output_type_id: TypeId::of::<BoxStream<'static, Result<ElemOut, Error>>>(),
            input_type_name: std::any::type_name::<BoxStream<'static, ElemIn>>(),
            output_type_name: std::any::type_name::<BoxStream<'static, Result<ElemOut, Error>>>(),
        });
        self
    }
...
impl<In, ElemIn> TypedChain<In, BoxStream<'static, ElemIn>>
where
    In: Send + Clone + 'static,
    ElemIn: Send + Clone + 'static,
{
    pub fn then_map_stream_in_with<ElemOut, P>(
        ...
        processor: P,
        concurrency: usize,
    ) -> TypedChain<In, BoxStream<'static, ElemOut>>
    where
        ElemOut: Send + 'static,
        P: ProcessorTrait<ElemIn, ElemOut> + Send + Sync + 'static,
    {
    ...
    pub fn then_map_stream_in_result_with<ElemOut, P>(
        ...
        processor: P,
        concurrency: usize,
    ) -> TypedChain<In, BoxStream<'static, Result<ElemOut, Error>>>
    where
        ElemOut: Send + 'static,
        P: ProcessorTrait<ElemIn, ElemOut> + Send + Sync + 'static,
    {
...
mod tests {
    ...
    fn init_logger() {
        INIT.call_once(|| {
            ...
            let _ = env_logger::Builder::from_env(env)
                .is_test(true) // 与测试输出整合，支持 -- --nocapture
                .try_init();
        });
    }
    ...
    // (i32,i32) -> BoxStream<i32>
    struct RangeToStream;
    #[async_trait]
    impl ProcessorTrait<(i32, i32), BoxStream<'static, i32>> for RangeToStream {
        fn name(&self) -> &'static str {
            "RangeToStream"
        }
        async fn process(
            ...
            input: (i32, i32),
            _ctx: ProcessorContext,
        ) -> ProcessorResult<BoxStream<'static, i32>> {
            let (start, end) = input;
            let st = futures::stream::iter(start..end).boxed();
            ProcessorResult::Success(st)
        }
    }

    // 收集逻辑在测试里直接对 BoxStream 进行 collect，不再作为处理器加入链

    #[tokio::test]
    async fn chain_stream_in_map_and_collect() {
        init_logger();
        let chain = TypedChain::<(i32, i32), (i32, i32)>::new()
            .then::<BoxStream<'static, i32>, _>(RangeToStream)
    ...
    async fn pipeline_file_sum_numbers() {
        ...
        // 运行：读取项目根目录下的 test.txt
        let ctx = ProcessorContext::default();
        let res = chain.execute("test.txt".to_string(), ctx).await;
        println!("{:#?}", res);
        matches!(res, ProcessorResult::Success(_));
    }
