...
use error::Result;
use error::error::RateLimitError;
use deadpool_redis::{
    Pool,
...
impl DistributedSlidingWindowRateLimiter {
    ...
    pub fn new(
        ...
        let key_prefix = format!("rate_limiter:{key_prefix}");
    ...
    fn get_lock_key(&self, identifier: &str) -> String {
        format!("{}:lock:{}", self.key_prefix, identifier)
    }
    ...
    pub async fn verify(&self, identifier: &str) -> Result<Option<u64>> {
            ...
            .await
        {
            ...
            // 获取最后请求时间
            let last_request_time: Option<u64> = conn
    ...
    /// * `Ok(())` - 未达到限制，已记录请求
    /// * `Err(u64)` - 达到限制，返回需要等待的毫秒数
    /// # 错误 当前记录的是最近的时间，do_download需要休眠   所以记录的时间戳需要加上休眠的时间
    /// # 弃用  使用verify和record分开调用
    async fn verify_and_record(&self, identifier: &str) -> Result<std::result::Result<(), u64>> {
        let config = self.get_key_config(identifier).await?;
        let lock_key = self.get_lock_key(identifier);
        let last_request_key = self.get_last_request_key(identifier);
            ...
            .await
        {
            ...
            // 必须加上外部休眠的时间戳  否则当前时间会比上次请求时间小
            let current_time = match result {
                Ok(_) => current_time,
                Err(t) => current_time.saturating_add(t),
            };
            // 间隔时间足够，记录新请求
            let _: () = conn
                .set_ex(&last_request_key, current_time, ttl_seconds)
